...
class TestCreateFunction(unittest.TestCase):
    ...
    def get_all_item(self, clear_password=False):
        get_all_cmd = " ".join(
            [self.get_prefix("GET"), self.endpoint, "-d output_mode=json"]
        )
        if clear_password:
    ...
    def get_all_passwords(self):
        get_password_cmd = " ".join(
            [self.get_prefix("GET"), self.password_endpoint, "-d output_mode=json"]
        )
    ...
    def testUpdateNoneExistedItem(self):
        ...
        update_cmd = " ".join(
            [self.get_prefix("POST"), self.endpoint + "/" + self.test_item_name, data]
        )
        output = subprocess.check_output(update_cmd.split(" "))
        self.assertTrue(
            'REST Error [404]: Not Found -- "{name}" does not exist'.format(
                name=self.test_item_name
            )
            in output
        )
    ...
    def testUpdateWithMultipleEmptyFieldValue(self):
        ...
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1= -d f2=")
        content = self.get_item_content()
        self.assertEqual(content["f1"], "")
        self.assertEqual(content["f2"], "")

        # update with f1='1' and f2='2'
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1=1 -d f2=2")
        content = self.get_item_content(clear_password=True)
    ...
    def testUpdateWithAdditionalFields(self):
        ...
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1=1 -d f2=2 -d f3=3")
    ...
    def testUpdateWithOneEmptyFieldValue(self):
        ...
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1=f1 -d f2=f2 -d f3=")
    ...
    def testUpdateWithPartialFields(self):
        ...
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1=f1 -d f2=")
    ...
    def testUpdateWithMixedRemove(self):
        ...
        self.update_item(data="-d endpoint=1 -d api_key=1 -d f1= -d f2= -d f3= -d f5=")
    ...
    def testGetWithOnePasswordChanged(self):
        ...
        self.update_item(
            data="-d api_key=other -d test_optional_password=other", via_conf=True
        )
    ...
    def testDeleteNoneExistentItem(self):
        ...
        output = subprocess.check_output(delete_cmd.split(" "))
        self.assertTrue(
            "Could not find object id={name}".format(name=self.test_item_name) in output
        )
